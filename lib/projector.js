...
var project = {
  ...
  'FeatureCollection': function(inProjection, outProjection, fc) {
    var from = projectorCache[inProjection];
    if (!from) {
      from = projectorCache[inProjection] = new Proj4js.Proj(inProjection);
    }
    var to = projectorCache[outProjection];
    if (!to) {
      to = projectorCache[outProjection] = new Proj4js.Proj(outProjection);
    }
    var _fc = __.clone(fc);
  ...
  'Point': function(inProjection, outProjection, c) {
    if (inProjection && outProjection) {
      ...
      if (inProjection instanceof Proj4js.Proj){
        from = inProjection;
      } else if (inProjection in projectorCache) {
        from = projectorCache[inProjection];
      } else {
        from = new Proj4js.Proj(inProjection);
        projectorCache[inProjection] = from;
      }

      if (outProjection instanceof Proj4js.Proj){
        to = outProjection;
      } else if (outProjection in projectorCache) {
        to = projectorCache[outProjection];
      } else {
        to = new Proj4js.Proj(outProjection);
        projectorCache[outProjection] = to;
      }
