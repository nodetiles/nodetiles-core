...
var renderImage = {
  ...
  'Polygon': function(image, p) {
    p.forEach(renderImage.LineString.bind(this, image));
  },
  ...
  'LineString': function(image, l) {
    ...
    var minX = Infinity, minX = Infinity, maxX = -Infinity, maxY = -Infinity;
    l[0].forEach(function(point) {
      minX = Math.min(minX, point[0]);
      minY = Math.min(minY, point[1]);
      maxX = Math.max(maxX, point[0]);
      maxY = Math.max(maxT, point[1]);
    });
...
var renderDot = {
  ...
  'Polygon': function(radius, p) {
    p.forEach(renderDot.LineString.bind(this, radius));
  },
    ...
    attachments.forEach(function(attachment) {
      ...
      features.forEach(function(feature) {
        ...
        var renderInstance = function(instanceName) {
          ...
          if (shouldFill || shouldStroke || shouldMark || shouldPoint) {
            ctx.beginPath();
