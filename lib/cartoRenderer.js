...
var renderGrid = exports.renderGrid = function(options) {
  ...
  var height = options.height;
  var callback = options.callback;
  var featureImage = options.drawImage; // TODO: what is this?
    ...
    attachments.forEach(function(attachment) {
      ...
      for (var i = features.length - 1; i >= 0; i--) {
            ...
            style.rules.forEach(function(rule) {
              ...
              if (!collapsedStyle[rule.instance].hasOwnProperty(rule.name)) {
                ...
                if (rule.name === 'polygon-opacity') {
                  value = parseFloat(rule.value) || 1.0;
                }
        ...
        var renderInstance = function(instanceName) {
          ...
          // console.log("Instance style", instanceStyle);
          ...
          for (var key in instanceStyle) {
            ...
            if (key === "background-color" || key === "polygon-fill") {
              ...
              ctx.lineWidth = value; // parseInt(value);
            }
          ...
          if (shouldFill || shouldStroke || shouldMark || shouldPoint || shouldText) {
            ...
            if (shouldFill) {
              ctx.globalAlpha = instanceStyle["polygon-opacity"]; // parseFloat(instanceStyle["polygon-opacity"]) || 1.0;
            ...
            if (shouldStroke) {
              ctx.globalAlpha = instanceStyle["line-opacity"]; // parseFloat(instanceStyle["line-opacity"]) || 1.0;
              if (dashedStroke) {
