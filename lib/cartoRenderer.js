...
var renderDashedPath = {
  ...
  'LineString': function(dashArray, l, offset) {
    ...
    var remainder = 0;
    var previousPoint = l[0];
    var transformedPrevious = roundPoint(transform(previousPoint));
    var prevAngle;
    for (var i = 1, len = l.length; i < len; i++) {
      var nextPoint = l[i];
      var transformedNext = roundPoint(transform(nextPoint));
      var dx = transformedNext[0] - transformedPrevious[0];
      var dy = transformedNext[1] - transformedPrevious[1];
      var lineLength = Math.sqrt(dx * dx + dy * dy);
      // only draw segments of 1px or greater
      if (lineLength >= 2 || i === len - 1) {
        remainder = renderDashedPath.Line.call(this, dashArray, previousPoint, nextPoint, -remainder);
        previousPoint = nextPoint;
        transformedPrevious = roundPoint(transform(previousPoint));
      }
    }
    
  },
  'Line': function(dashArray, start, end, offset) {
    // var patternLength = dashArray.reduce(function(sum, item) { return (sum || 0) + item; });
    // console.log("Drawing", dashArray, " with offset:", offset, " on pattern length:", patternLength);
    // we're gonna do some transforms
    this.save();
    
    var start = roundPoint(transform(start));
    var end = roundPoint(transform(end));
    var dx = end[0] - start[0],
        dy = end[1] - start[1];
    
    var innerTransform = function(point) {
      return [point[0], point[1] - 0.5];
      return [point[0], point[1] + 0.5 - 10];
      return [point[0] + start[0] + 10, point[1] + start[1] - 10];
    }
    ...
    if (length < 1) {
      // console.log("LENGTHENING TO ONE");
      length = 1;
    }
    ...
    // var pt = innerTransform([0, -4]);
    // this.moveTo(pt[0] + 0.5, pt[1]);
    // pt = innerTransform([0, -14]);
    // this.lineTo(pt[0] + 0.5, pt[1]);
    // 
    // for (var i = 8 + (offset || 0); i < length + 8; i += 8) {
    //   pt = innerTransform([i, -8]);
    //   this.moveTo(pt[0] + 0.5, pt[1]);
    //   pt = innerTransform([i, -14]);
    //   this.lineTo(pt[0] + 0.5, pt[1]);
    // }
    
    pt = innerTransform([decOffset, 0]);
    ...
    this.restore();
    // var patternLength = dashArray.reduce(function(sum, item) { return (sum || 0) + item; });
    // console.log("Pattern distance:", patternDistance, " remainder:", (patternLength - patternDistance));
